...
const ProductForm: React.FC<ProductFormProps> = ({ productToEdit, onFormSubmit }) => {
  ...
  const [productData, setProductData] = useState({
    ...
    price: '',
    promoPrice: '',
    category: '', // Categoria inicial vazia ou a primeira disponível
    imageUrl: '',
    stock: '0',
  });
  ...
  useEffect(() => {
    if (productToEdit) {
      ...
      setProductData({
        ...
        category: categories.length > 0 ? categories[0].name : '', // Define a primeira categoria como padrão
        imageUrl: '', stock: '0',
      });
      setSpecifications([]);
      setImageFile(null);
  ...
  }, [productToEdit, categories]); // Adicionar categories como dependência
