...
import { getLucideIcon } from '../lib/lucide-icon-map'; // Importar getLucideIcon
...
const CategoryNav: React.FC<CategoryNavProps> = ({ selectedCategory, onSelectCategory }) => {
  const { categories } = useCategories(); // Usar categorias do contexto
  ...
  }, [categories]); // Adicionar categories como dependência
            ...
            {categories.map(category => {
              ...
              return (
                <button
                  ...
                  className={getButtonClasses(selectedCategory === category.name)}
                >
